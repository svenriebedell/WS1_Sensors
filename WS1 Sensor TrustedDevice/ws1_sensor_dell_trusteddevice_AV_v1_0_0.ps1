##################################################################
#
# Name: Sensor Dell TrustedDevice for Antivirus status
#
...
$AV ="UNKNOWN"
foreach ($Line in $Message.Split([Environment]::NewLine) )     {     if ( $Line.contains("Antivirus solution detected and enabled: ") ) { $AV = ($Line.Split(" "))[-1] }     }
Write-Output $AV