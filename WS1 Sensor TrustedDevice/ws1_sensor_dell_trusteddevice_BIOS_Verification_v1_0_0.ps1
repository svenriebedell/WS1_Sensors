##################################################################
#
# Name: Sensor Dell TrustedDevice for BIOS verification status
#
...
$BIOSverify ="UNKNOWN"
foreach ($Line in $Message.Split([Environment]::NewLine) )     {     if ( $Line.contains("BIOS Verification: ") ) { $BIOSverify = ($Line.Split(" "))[-1] }     }
Write-Output $BIOSverify