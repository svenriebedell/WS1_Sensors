##################################################################
#
# Name: Sensor Dell TrustedDevice for ME verification status
#
...
$MEverify ="UNKNOWN"
foreach ($Line in $Message.Split([Environment]::NewLine) )     {     if ( $Line.contains("ME Verification: ") ) { $MEverify = ($Line.Split(" "))[-1] }     }
Write-Output $MEverify