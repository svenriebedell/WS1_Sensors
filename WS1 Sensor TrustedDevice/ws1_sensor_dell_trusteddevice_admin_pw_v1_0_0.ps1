##################################################################
#
# Name: Sensor Dell TrustedDevice for admin password is set or not
#
...
$APW ="UNKNOWN"
foreach ($Line in $Message.Split([Environment]::NewLine) )     {     if ( $Line.contains("BIOS Admin Password set:") ) { $APW = ($Line.Split(" "))[-1] }     }
Write-Output $APW