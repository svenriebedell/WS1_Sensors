##################################################################
#
# Name: Sensor Dell TrustedDevice for indicators of Attack (IoA)
#
...
$IOA ="UNKNOWN"
foreach ($Line in $Message.Split([Environment]::NewLine) )     {     if ( $Line.contains("Indicators of Attack detected:") ) { $IOA = ($Line.Split(" "))[-1] }     }
Write-Output $IOA