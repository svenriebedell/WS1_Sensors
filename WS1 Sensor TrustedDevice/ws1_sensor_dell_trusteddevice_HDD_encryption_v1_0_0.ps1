##################################################################
#
# Name: Sensor Dell TrustedDevice for disk encryption status
#
...
$HDencrypt ="UNKNOWN"
foreach ($Line in $Message.Split([Environment]::NewLine) )     {     if ( $Line.contains("Disk Encryption: ") ) { $HDencrypt = ($Line.Split(" "))[-1] }     }
Write-Output $HDencrypt